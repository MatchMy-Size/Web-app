...
import { createUserWithPhonePassword, signInWithPhonePassword } from '@/lib/firebase-auth';
...
export function OtpPage() {
  ...
  const handleVerify = async (e: React.FormEvent) => {
    ...
    try {
      ...
      try {
        await createUserWithPhonePassword(registration.phoneNumber, registration.password);
      } catch (err) {
        if (!isEmailAlreadyInUseError(err)) throw err;
        await signInWithPhonePassword(registration.phoneNumber, registration.password);
      }
      ...
      const signedInUser = await signInWithPhonePassword(registration.phoneNumber, registration.password)
        .then(r => r.user).catch(() => null);
