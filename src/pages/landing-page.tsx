...
const CSS = `
  ...
  .fb-scene { position: relative; width: 100%; height: 480px; overflow: hidden; }
  .fb-logo {
    position: absolute;
    display: flex; align-items: center; justify-content: center;
    background: var(--white);
    ...
    transition: transform 0.38s cubic-bezier(0.22,1,0.36,1), border-color 0.22s, box-shadow 0.22s;
  ...
  .fb-logo:hover {
    transform: translate(var(--fb-tx,0px), var(--fb-ty,0px)) scale(1.2) !important;
...
/* ═══════════════════════════════════════════════
   FLOATING BRANDS
═══════════════════════════════════════════════ */
type FBrand = { id: string; w: number; h: number; render: () => ReactNode };

const FB_BRANDS: FBrand[] = [
  { id:'hm',      w:96,  h:56, render:()=><img src={hmBrand} alt="H&M" /> },
  { id:'zara',    w:112, h:50, render:()=><img src={zaraBrand} alt="Zara" /> },
  { id:'adidas',  w:104, h:58, render:()=><img src={adidasBrand} alt="Adidas" /> },
  { id:'tommy',   w:110, h:56, render:()=><img src={tommyHilfigerBrand} alt="Tommy Hilfiger" /> },
  { id:'gap',     w:88,  h:50, render:()=><img src={gapBrand} alt="Gap" /> },
  { id:'uniqlo',  w:114, h:50, render:()=><img src={uniqloBrand} alt="Uniqlo" /> },
  { id:'spring',  w:124, h:50, render:()=><img src={springAndSummerBrand} alt="Spring & Summer" /> },
  { id:'pb',      w:118, h:50, render:()=><img src={pullAndBearBrand} alt="Pull&Bear" /> },
  { id:'levis',   w:102, h:50, render:()=><img src={levisBrand} alt="Levi's" /> },
  { id:'puma',    w:98,  h:50, render:()=><img src={pumaBrand} alt="Puma" /> },
  { id:'mimosa',  w:102, h:50, render:()=><img src={mimosaBrand} alt="Mimosa" /> },
  { id:'mango',   w:116, h:50, render:()=><img src={mangoBrand} alt="Mango" /> },
  { id:'ms',      w:82,  h:50, render:()=><img src={marksAndSpencerBrand} alt="Marks & Spencer" /> },
  { id:'bershka', w:112, h:50, render:()=><img src={bershkaBrand} alt="Bershka" /> },
  { id:'ck',      w:94,  h:56, render:()=><img src={calvinKleinBrand} alt="Calvin Klein" /> },
];

const FB_PAD = 18;

function fbPlace(W: number, H: number) {
  const placed: { x:number; y:number; w:number; h:number }[] = [];
  return FB_BRANDS.map(b => {
    let pos: {x:number;y:number}|null = null;
    for (let i=0; i<400; i++) {
      const x = FB_PAD + Math.random()*(W-b.w-FB_PAD*2);
      const y = FB_PAD + Math.random()*(H-b.h-FB_PAD*2);
      if (!placed.some(p=>x<p.x+p.w+FB_PAD&&x+b.w+FB_PAD>p.x&&y<p.y+p.h+FB_PAD&&y+b.h+FB_PAD>p.y)) {
        placed.push({x,y,w:b.w,h:b.h}); pos={x,y}; break;
      }
    }
    if (!pos) { pos={x:FB_PAD+(placed.length%4)*150,y:FB_PAD+Math.floor(placed.length/4)*80}; placed.push({x:pos.x,y:pos.y,w:b.w,h:b.h}); }
    return { ...b, x:pos.x, y:pos.y,
      ampY:5+Math.random()*9, ampX:3+Math.random()*5,
      speedY:2800+Math.random()*3200, speedX:3600+Math.random()*2800,
      phaseY:Math.random()*Math.PI*2, phaseX:Math.random()*Math.PI*2,
...
function FloatingBrands() {
  ...
  const dataRef  = useRef<ReturnType<typeof fbPlace>>([]);
  ...
  useEffect(() => {
    ...
    const H = 480;
    dataRef.current = fbPlace(W, H);
    dataRef.current.forEach(d => {
      const el = scene.querySelector<HTMLDivElement>(`[data-fbid="${d.id}"]`);
      if (el) { el.style.left=`${d.x}px`; el.style.top=`${d.y}px`; el.style.zIndex=String(d.z); elMap.current.set(d.id,el); }
    });
    let t0: number|null = null;
    const tick = (ts: number) => {
      if (!t0) t0=ts; const t=ts-t0;
      dataRef.current.forEach(d => {
        const el=elMap.current.get(d.id); if (!el) return;
        const ty=Math.sin((t/d.speedY)*Math.PI*2+d.phaseY)*d.ampY;
        const tx=Math.sin((t/d.speedX)*Math.PI*2+d.phaseX)*d.ampX;
        el.style.setProperty('--fb-tx',`${tx}px`);
        el.style.setProperty('--fb-ty',`${ty}px`);
  ...
  return (
    <div className="fb-scene" ref={sceneRef} style={{height:480}}>
      {FB_BRANDS.map(b=>(
        <div key={b.id} data-fbid={b.id} className="fb-logo" style={{width:b.w,height:b.h}}>
          <b.render/>
        </div>
      ))}
    </div>
